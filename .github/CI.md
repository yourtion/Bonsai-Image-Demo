...
|-----|--------|----------------|
| `macos-apple-silicon` | `macos-14` (GitHub-hosted) | Full e2e: `setup.sh` → download ternary model → `generate.sh` small image → `check_image.py` pixel quality check (not all-black, not constant noise) → image uploaded as artifact → `serve.sh` smoke (`/backends` + frontend root, then stop). |
